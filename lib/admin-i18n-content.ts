...
function launchMessageZh(signal: StatusText) {
  ...
  switch (signal.key) {
    ...
    case "shipping-rates":
      return "当前结账仅收取商品款；运费由客服另行核算，待客户确认后在网站外收取。Melhor Envio 未用于结账，其配置不会阻塞付款。";
...
const readinessItemsZh: Record<string, { group: string; title: string; description: string }> = {
  ...
  "shipping-melhor-envio": { group: "物流", title: "运费政策与未来物流接入", description: "当前网站仅收商品款，运费由客服另行核算并在网站外收取；Melhor Envio 配置仅作为未来可选集成。" },
